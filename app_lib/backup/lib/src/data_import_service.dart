...
class DataImportService {
  ...
  Future<Map<String, dynamic>> parseZipFile(File zipFile) async {
    try {
      ...
      try {
        DataValidator.validateManifest(manifest);
      } catch (e) {
  ...
  ///
  /// This method wraps the entire import operation in a database transaction
  /// to ensure atomicity. If any part of the import fails, all changes are rolled back.
    ...
    try {
      ...
      try {
        ...
        if (await tempDir.exists()) {
          ...
          AppLogger().d('Cleaned up temporary directory: $extractDir');
        }
      } catch (cleanupError) {
        AppLogger().w('Failed to cleanup temp directory: $cleanupError');
      }
    }
  }
      ...
      switch (resolution) {
        ...
        case ConflictResolution.override:
          ...
          for (final visit in existingVisits) {
            ...
            // Delete the visit
            await _database.deleteVisit(visit.id);
          }
          // Now safe to delete treatment
  ...
  Future<int?> _importHospital(Map<String, dynamic> hospitalJson) async {
    ...
    DataValidator.validateHospital(hospitalJson);
    ...
    if (existing != null) {
      AppLogger().d('Reusing existing hospital: $name (ID: ${existing.id})');
      return existing.id;
    }

    // Create new hospital, preserving departmentIds if present
    final departmentIds = hospitalJson['departmentIds'] as String? ?? '[]';
    ...
    return await _database.createHospital(
      HospitalsCompanion(
        ...
        departmentIds: drift.Value(departmentIds),
      ),
    );
  }
  ...
  Future<int?> _importDepartment(Map<String, dynamic> departmentJson, int hospitalId) async {
    ...
    if (existing != null) {
      AppLogger().d('Reusing existing department: $name (ID: ${existing.id})');
      return existing.id;
    }
  ...
  Future<int?> _importDoctor(
    ...
    DataValidator.validateDoctor(doctorJson);
    ...
    if (existing != null) {
      AppLogger().d('Reusing existing doctor: $name (ID: ${existing.id})');
      return existing.id;
    }

    // departmentId is required for doctors table
    if (departmentId == null) {
      AppLogger().w('Cannot create doctor without department ID: $name');
      return null;
    }
  ...
  Future<void> _importVisit(
    ...
    final visitId = await _database.createVisit(
      VisitsCompanion(
        ...
        informations: drift.Value(visitJson['informations'] as String?),
      ),
    );

    AppLogger().d('Created visit ID: $visitId for treatment $treatmentId');
  }
  ...
  Future<void> _importResource(
    ...
    DataValidator.validateResource(resourceJson);
