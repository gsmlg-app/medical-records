...
void main() {
  ...
  group('Export/Import Round-Trip Integration Tests', () {
    test(
      'Complete treatment with all relationships should survive round-trip',
      () async {
        ...
        final deletedTreatment = await database.getTreatmentById(
          fixture.treatment.id,
        );
        ...
        final conflicts = await importService.checkConflicts(
          parsedData['manifest'],
        );
        ...
        expect(
          TestHelpers.datesEqual(
            importedTreatment.startDate,
            fixture.treatment.startDate,
          ),
          isTrue,
        );
        expect(
          TestHelpers.datesEqual(
            importedTreatment.endDate,
            fixture.treatment.endDate,
          ),
          isTrue,
        );

        // Verify visits
        final importedVisits = await database.getVisitsByTreatment(
          importedTreatment.id,
        );
        ...
        expect(
          importedHospitals.first.departmentIds,
          fixture.hospital.departmentIds,
        );
    ...
    test(
      'Multiple treatments should maintain separation after round-trip',
      () async {
        ...
        final visits1 = await database.getVisitsByTreatment(
          allTreatments[0].id,
        );
        final visits2 = await database.getVisitsByTreatment(
          allTreatments[1].id,
        );
    ...
    test(
      'Visit with all optional fields null should survive round-trip',
      () async {
        // ARRANGE: Create treatment with minimal visit
        final treatmentId = await database.createTreatment(
          SampleData.sampleTreatment1(),
        );

        await database.createVisit(
          SampleData.visitWithNulls(treatmentId: treatmentId),
        );
        ...
        final allTreatments = await database.getAllTreatments();
        final visits = await database.getVisitsByTreatment(
          allTreatments.first.id,
        );
    ...
    test(
      'Hospital departmentIds should be preserved during round-trip',
      () async {
        // ARRANGE: Create hospital with department IDs
        final hospitalId = await database.createHospital(
          SampleData.sampleHospital1(),
        );
        ...
        expect(
          importedHospitals.first.departmentIds,
          originalHospital!.departmentIds,
        );
    ...
    test(
      'Large dataset (10 treatments) should complete without errors',
      () async {
    ...
    test(
      'Empty treatment list should create valid export with zero treatments',
      () async {
